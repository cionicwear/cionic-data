...
def main():
    ...
    parser.add_argument('-c', dest='csvs', nargs="+", required=False, help='generate CSV for listed streams')
    parser.add_argument('-f', dest='files', action='store_true', help='download additional collection files')
    parser.add_argument('-l', dest='limit', default=20, type=int, help='number of most recent collections to fetch')
    parser.add_argument('-n', dest='nums', nargs='+', type=int, required=False, help='collection numbers to download')
    parser.add_argument('-o', dest='outdir', default="./recordings", help='directory to store output')
    parser.add_argument('-p', dest='protoid', type=str, required=False, help='protocol shortname to download')
    ...
    if args.protoid is None:
        print(f"Fetching [{args.limit}] collections for org [{args.orgid}] study [{args.studyid}] all protocols")
        for p in protocols:
            print(f"  {p['shortname']}")
        collections = cionic.get_cionic(f"{args.orgid}/collections?sxid={sxid}")
    elif pxid := named_protos.get(args.protoid):
        print(f"Fetching [{args.limit}] collections for org [{args.orgid}] study [{args.studyid}] proto [{args.protoid}]")
        collections = cionic.get_cionic(f"{args.orgid}/collections?protoxid={pxid}")
    else:
        print(f"Protocol [{args.protoid}] not found for org [{args.orgid}] study [{args.studyid}]")
        return
