...
def get_walking_intervals(
    ...
    for grouped_splits in all_grouped_splits:
        ...
        if len(grouped_splits) + 1 > n_stop_remove:
            grouped_splits = (
                grouped_splits[:-n_stop_remove] if n_stop_remove > 0 else grouped_splits
            )
...
class Kinematics:
    ...
    def load_csv_stream(
        self, group, position, stream, array, label_range=None, time_range=None
    ):
        print(f"loading {position} {stream} to {group}")
        if label_range is not None:
            array = array[
                (array['elapsed_s'] >= label_range[0])
                & (array['elapsed_s'] < label_range[1])
            ]
    ...
    def check(self, group, position=None, stream=None, component=None):
        ...
        if (
            component
            and component not in self.groups[group][position][stream].dtype.names
        ):
    ...
    def split_stream(self, split_name, group, position, stream, component):
        ...
        for i, strike in enumerate(split['splits']):
            ...
            if i not in split['skips']:
                splits.append(
                    {component: sub[component], 'elapsed_s': sub['elapsed_s']}
                )
        return splits
    ...
    def calculate_split(
        ...
        for split in splits:
            ...
            if component == 'emg' and rms_emg_after_seg.get('action', False):
                ...
                split[component] = tools.moving_avg_rms(
                    split[component], window_size=rmsw
                )
                if norm:
                    split[component] = split[component] / norm(split[component])

            (time_interp, step_interp) = self.interpolate_split(
                split[component], target_length
            )
            steps = np.append(steps, [step_interp], axis=0)
            steps_ind.append((time_interp, step_interp))

        return {'avg': steps.mean(axis=0), 'std': steps.std(axis=0), 'ind': steps_ind}
    ...
    def normativescore(self, k, norm_kin_data_dict, groups, planes, split):
        ...
        for side in groups:
            for angle in planes:
                normdf[(side + '_' + angle)] = norm_kin_data_dict[side][angle][
                    'avg_angle'
                ]
        normdf = normdf[0:100]
        ...
        for side in groups:
            for angle in planes:
                name = side + '_' + angle
                d = cdist_dtw(
                    np.squeeze(np.array(normdf[name])),
                    np.squeeze(np.array(imudf[name])),
                )
                du = (1 - (d / 2000)) * 100
                if du < 0:
                    du = 0
        ...
        for side in groups:
            for angle in planes:
                ...
                if splits:
                    ...
                    sd_score = np.std(std)
                    sim_score = (
                        1 - (((sd_score - 0) * (1 - 0)) / (2 - 0)) + 0
                    ) * 100  # Assumes maximum mean SD possible is 2 degrees
                    angles.append(angle)
                    sides.append(side)
    ...
    def symmetryscore(self, k, groups, planes, split):
        ...
        if len(groups) < 2:
            ...
            for angle in planes:
                imudf = pd.DataFrame()
                for side in groups:
                    splits = k.export_split(
                        split, side, angle, 'angle', 'degrees', points
                    )
                    if splits:
                        df_imu = pd.DataFrame(splits)
                        df_imu.columns = ['start', 'duration'] + list(range(0, points))
                ...
                d = cdist_dtw(
                    np.squeeze(np.array(imudf['right'])),
                    np.squeeze(np.array(imudf['left'])),
                )
                du = (1 - (d / 1000)) * 100
                if du < 0:
                    du = 0
    ...
    def stat_tests(self, k, groups, planes, split):
        ...
        for c in ['Max_Full', 'Min_Full', 'MaxC_Full', 'MinC_Full', 'ROM_Full']:
            for a in scoretable.Angle.unique():
                right = scoretable.loc[
                    (scoretable['Angle'] == a) & (scoretable['Side'] == 'right'), c
                ].values[0]
                left = scoretable.loc[
                    (scoretable['Angle'] == a) & (scoretable['Side'] == 'left'), c
                ].values[0]
    ...
    def normgaitscore(self, gaitscores, norm_kin_data_dict, groups, planes):
        ...
        scoretable['Compare Max. (%)'] = round(
            scoretable.apply(
                lambda row: self.asi(row['Normative Max.'], row['Participant Max.']),
                axis=1,
            ),
            2,
        )
        ...
        scoretable['Compare Min. (%)'] = round(
            scoretable.apply(
                lambda row: self.asi(row['Normative Min.'], row['Participant Min.']),
                axis=1,
            ),
            2,
        )
        ...
        scoretable['Compare % Cycle Max. (%)'] = round(
            scoretable.apply(
                lambda row: self.asi(
                    row['Normative % Cycle Max.'], row['Participant % Cycle Max.']
                ),
                axis=1,
            ),
            2,
        ...
        scoretable['Compare % Cycle Min. (%)'] = round(
            scoretable.apply(
                lambda row: self.asi(
                    row['Normative % Cycle Min.'], row['Participant % Cycle Min.']
                ),
                axis=1,
            ),
            2,
        ...
        scoretable['Compare ROM (%)'] = round(
            scoretable.apply(
                lambda row: self.asi(row['Normative ROM'], row['Participant ROM']),
                axis=1,
            ),
            2,
        )
        ...
        for col, group in enumerate(groups):
            ...
            if group in markers:
                for mark in markers[group]:
                    line = axs.axvline(
                        mark['x'], label=mark['label'], color=mark['color']
                    )
                    if 'stdev' in mark:
                        axs.axvspan(
                            mark['x'] - mark['stdev'],
            ...
            if plot_ind:
                ...
                for position, ind in individuals.items():
                    ...
                    if norm_emg_data is not None:
                        ...
                        axs.fill_between(
                            x_percent_norm,
                            np.subtract(emg_norm_avg, emg_norm_std),
                            np.add(emg_norm_avg, emg_norm_std),
                            color='lightgrey',
                            alpha=0.5,
                        )
                    ...
                    if norm_kin_data is not None:
                        pos = position[
                            0 : -1 - 5
                        ]  # get rid of the '_angles' part of string position

                        x_percent_norm = (
                            np.array(norm_kin_data[group][pos]['gait_cycle']) / 100
                        )  # values = decimals, labels = %
                        ...
                        axs.fill_between(
                            x_percent_norm,
                            np.subtract(avg_norm, std_norm),
                            np.add(avg_norm, std_norm),
                            color='lightgrey',
                            alpha=0.5,
                        )
        self.save_fig(fig)
            ...
            for position in positions:
                    ...
                    streams = list(
                        set(
                            functools.reduce(
                                operator.iconcat,
                                [
                                    list(self.groups[group][pos].keys())
                ...
                for stream in streams:
                    ...
                    comps = (
                        components
                        if components
                        else [k for k in ndkeys if k != 'elapsed_s']
                    )
                    for component in comps:
    ...
    def plot_muscle_wheel(self, split, mconfig, step, mult, width, height):
        ...
        fig, axes = plt.subplots(
            ncols=2, nrows=1, constrained_layout=True, subplot_kw={'polar': True}
        )
        fig.set_size_inches(width, height, forward=False)
    ...
    def orientations_for_single_stream(self, data, calibration=None):
        ...
        orientation_dict = {'orientation': orientation, 'elapsed_s': data["elapsed_s"]}
    ...
    def calculate_joint_angle(self, group, position_a, position_b, stream):
        ...
        data_b, calibration_b = self.get_data_array_and_calibration(
            group, position_b, stream
        )
        ...
        orientations_b = self.orientations_for_single_stream(
            data_b, calibration=calibration_b
        )  # dict with many Rotations and time array

        elapsed_s_interp = self.get_synthetic_interpolated_array(
            orientations_a["elapsed_s"], orientations_b["elapsed_s"]
        )
        ...
        for idx in range(elapsed_s_interp.shape[0]):
            ...
            euler_sample = tuple(self.calculate_angle(q_a, q_b, 'xyz')) + (
                elapsed_s_interp[idx],
            )
    ...
    def create_upright(self, group, angles, stream):
        for joint, angles in angles.items():
            ...
            if self.check(group, position_b, stream):
                self.groups[group]['upright'][stream] = self.create_upright_stream(
                    self.groups[group][position_b][stream]
                )
                return
    ...
    def calculate_limb_angles(self, group, positions, stream='fquat'):
        ...
        for position in positions:
            ...
            data, calibration = self.get_data_array_and_calibration(
                group, position, stream
            )

            orientations = self.orientations_for_single_stream(
            ...
            self.groups[group][position]['euler'] = np.array(
                euler_stream,
                dtype={
                    'names': ('x', 'y', 'z', 'elapsed_s'),
                    'formats': ('f8', 'f8', 'f8', 'f8'),
    ...
    def emg_fields(self, group, position, stream="emg"):
        if stream in self.groups[group][position]:
            return [
                fld
                for fld in self.groups[group][position][stream].dtype.fields
                if fld != 'elapsed_s'
            ]
        else:
            return []
    ...
    def calculate_emgs(self, group, positions, emg_params, stream="emg"):
        ...
        for position in positions:
            ...
            for i, field in enumerate(fields):
                ...
                arr = np.array(
                    list(zip(output, elapsed)),
                    dtype={'names': ('emg', 'elapsed_s'), 'formats': ('f8', 'f8')},
                )
    ...
    def calculate_pressures(
        ...
        for position in positions:
            ...
            if stream in self.channels[group][position]:
                channel_map = [
                    ch for ch in self.channels[group][position][stream] if ch
                ]
                # protect against bad mapping
                if len(channel_map) != len(fields):
                    logging.error(
                        f"CHANPOS {channel_map} does not match FIELDS {fields}"
                    )
                    channel_map = fields
            else:
                channel_map = fields
            ...
            for i, field in enumerate(fields):
                ...
                self.groups[group][position][cpos] = np.array(
                    list(zip(output, elapsed)),
                    dtype={
                        'names': (component_out, 'elapsed_s'),
                        'formats': ('f8', 'f8'),
                    },
                )
    ...
    def calculate_fes(
        ...
        for position, component_map in positions.items():
            for component, remap in component_map.items():
                ...
                self.groups[group][remap][stream_out] = np.array(
                    list(zip(vals, times)),
                    dtype={
                        'names': (component_out, 'elapsed_s'),
                        'formats': ('f8', 'f8'),
    ...
    def signal_at_splits(
        self, splits, group, position, stream, component, ts="elapsed_s"
    ):
        points = []
        if not self.check(group, position, stream, component):
            return points
    ...
    def add_knee_angles(self, splits, side, signals, time_key, entry_key):
        knee_angles = self.signal_at_splits(
            splits, side, 'knee_flexion', 'angle', 'degrees'
        )
        knee_angle_d = {}
    ...
    def gait_cycles(self, side, thigh, shank):
        ...
        heel_strike_shank = self.signal_at_splits(
            heel_strikes, side, shank, 'euler', 'x'
        )
        heel_strike_thigh = self.signal_at_splits(
            heel_strikes, side, thigh, 'euler', 'x'
        )
        ...
        for i, hs_shank in enumerate(heel_strike_shank):
            ...
            while (
                toe_index < len(toe_off_shank)
                and toe_off_shank[toe_index][0] < hs_shank[0]
            ):
                toe_index += 1
        ...
        for i, to_shank in enumerate(toe_off_shank):
            ...
            while (
                heel_index < len(heel_strike_shank)
                and heel_strike_shank[heel_index][0] < to_shank[0]
            ):
                heel_index += 1
        ...
        for i, stance in enumerate(stances):
            ...
            while (
                ...
                and swings[swing_index]['to_ts'] < stance['to_ts']
            ):
                swing_index += 1
            ...
            assert (
                stance['to_ts'] == swing['to_ts']
            )  # make sure we are properly matched
            cycles.append(
                {
    ...
    def gait_triangles(self, cycles, thigh_len, shank_len):
        for cycle in cycles:
            ...
            swing_hs_tri = costri(
                shank_len, thigh_len, math.radians(180 - cycle['swing_hs_ka'])
            )
    ...
    def stride_len_simple(self, sdf, field):
        ...
        sdf[field] = (
            stance_factor * sdf["stance_trans_len"]
            + swing_factor * sdf["swing_trans_len"]
            + swing_hip_factor * sdf["swing_hip_trans"]
        )
