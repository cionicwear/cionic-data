...
def butter_highpass(filter_order, cutoff, sampling_freq):
    b, a = signal.butter(filter_order, cutoff, btype='highpass', analog=False, fs = sampling_freq)
...
def process_raw_emg(stream: np.ndarray, hp_filter=True, rectify=True, cutoff_spikes=True, max_norm=True, rms=True, n_cutoff_std=15, hp_params=HP_PARAMS, rms_params=RMS_PARAMS):
    if hp_filter:
...
def process_raw_emg_stream(stream_timestamped, hp_filter=True, rectify=True, cutoff_spikes=False, max_norm=True, rms=True, n_cutoff_std=15, hp_params=HP_PARAMS, rms_params=RMS_PARAMS):
    ...
    dtype = {"names": tuple(fields + ["elapsed_s"]), "formats": tuple(["f8"] * (len(fields) + 1))}
...
def convert_uV(raw_data, v_ref, channel_gain):
    ...
    """
    data_uV = 1e6 * raw_data * v_ref / (channel_gain * (2**23 - 1)) # units: microvolts (uV)
...
def compute_threshold_crossings(stream, threshold=0, crossing_type="both"):
    if crossing_type == "both":
        pos_crossings = compute_threshold_crossings(stream, threshold=threshold, crossing_type="positive")
        neg_crossings = compute_threshold_crossings(stream, threshold=threshold, crossing_type="negative")
...
def simple_plot(streams, components=None, off=0, width=5, height=5, y_column='elapsed_s', title='', xlabel='', ylabel='', color='', ylim=[0,0], leg_contents=[], style='-', legend_loc='best'):
    fig, axs = plt.subplots(ncols=1, nrows=1, constrained_layout=True)
    fig.set_size_inches(width, height, forward = False)

    component_plot(axs, streams, components=components, off=off, y_column=y_column,
                    title=title, xlabel=xlabel, ylabel=ylabel, color=color, ylim=ylim,
                    leg_contents=leg_contents, style=style, legend_loc=legend_loc)
    ...
    for shade in shades:
        ...
        for pattern in shade['patterns']:
            if pattern[0] == pattern[1]:
                axs.vlines(pattern[0], ylims[0], ylims[1], color=color, alpha=pattern[2])
            else:
                plot = axs.fill_between(pattern[0:2], ylims[0], ylims[1], color=color, alpha=pattern[2], linewidth=0)
                if first:
                    plots.append(plot)
                    legend.append(name)
                    first = False

def component_plot(axs, streams, components=None, off=0, y_column='elapsed_s',
                title='', xlabel='', ylabel='', color='', ylim=[0,0],
                leg_contents=[], style='-', legend_loc='best', shades=[]):
    ...
    for idx, stream in enumerate(streams):
        if color:
            (offset, plot) = _plot(stream, components, offset, off, axs, legend, y_column, color=color, style=style)
        else:
            (offset, plot) = _plot(stream, components, offset, off, axs, legend, y_column, style=style)
...
def configurable_plot(streams, components=None, off=0, width=10, height=7, y_column='elapsed_s',
                      title='', xlabel='', ylabel='', color='', ylim=[0,0],
                      ncols=1, same_plot=True, leg_contents=[], style='-', legend_loc='upper right',
                      sharex=True, shades=[]):
    if same_plot:
        fig, axs = plt.subplots(ncols=1, nrows=1, constrained_layout=True)
        fig.set_size_inches(width, height, forward = False)
        ...
        component_plot(axs, streams, components=components, off=off, y_column=y_column,
                    title=title, xlabel=xlabel, ylabel=ylabel, color=color, ylim=ylim,
                    leg_contents=leg_contents, style=style, legend_loc=legend_loc, shades=shades)
        ...
        for idx, stream in enumerate(streams):
            ...
            component_plot(axs, [stream], components=components, off=0, y_column=y_column,
                        title=title, xlabel=xlabel, ylabel=ylabel, color=color, ylim=ylim,
                        leg_contents=[legend], style=style, legend_loc=legend_loc, shades=shades)
...
def join_segments(npz, segments):
    ...
    for index, seg in segments.iterrows():
        ...
        if key in data:
            ...
            data[key] = {
                'stream' : stream,
                'dtype' : npz[seg['path']].dtype
            }

            if seg.get('calibration'):
                data[key]['calibration'] = struct.unpack("<5f", eval(seg['calibration']))
    return data

def regs_data(npz, regpath):
    regs = pd.DataFrame(npz[regpath])
    # convert the string columns from bytes to strings
    str_df = regs[['regname','desc','parsedval']].stack().str.decode('utf-8').unstack()
...
def stream_calquat(stream, calibration):
    ...
    return np.array(quats, dtype={'names': ('x', 'y', 'z', 'w', 'elapsed_s'),
...
def stream_quat2euler(stream, calibration=None, degrees=True):
    ...
    return np.array(eulers, dtype={'names': ('x', 'y', 'z', 'elapsed_s'),
                                   'formats':('f8', 'f8', 'f8', 'f8')})
...
def stream_quat2euler_joint(quat_stream_1, quat_stream_2):
    ...
    if n_1 != quat_stream_1.shape[0]:
        print(f"quat_stream_1 {n_1 - quat_stream_1.shape[0]} nonincreasing samples removed (of {quat_stream_1.shape[0]} total samples).")
    if n_2 != quat_stream_2.shape[0]:
        print(f"quat_stream_2 {n_2 - quat_stream_2.shape[0]} nonincreasing samples removed (of {quat_stream_2.shape[0]} total samples).")
    ...
    orientations_1 = orientations_for_single_stream(quat_stream_1, calibration=calibration_1) # dict with many Rotations and time array
    orientations_2 = orientations_for_single_stream(quat_stream_2, calibration=calibration_2) # dict with many Rotations and time array

    elapsed_s_interp = get_interpolated_array_from_two_arrays(orientations_1["elapsed_s"], orientations_2["elapsed_s"])
    ...
    eulers_vectorized = np.array(eulers_tupled, dtype={'names': ('x', 'y', 'z', 'elapsed_s'), 'formats':('f8', 'f8', 'f8', 'f8')})
...
def stream_data(npz, streams, degrees=True):
    ...
    for index, seg in pd.DataFrame(npz['segments']).iterrows():
        ...
        if device_stream not in data:
            ...
            if not p:
                p = "."
            components += [ f"{device_name} {stream_name} {f} {p}" for f in stream.dtype.names if f != "elapsed_s" ]
        else:
            data[device_stream]['stream'].extend(stream.tolist())

    return { k : np.array(v['stream'], dtype=v['dtype']) for k,v in data.items() }, times, components
...
def get_stream_data_joints(npz, streams):
    ...
    for joint in joints:
        for side in sides:
            ...
            if f'{side[0]}_{limb_comp_1}' in device_dict.keys() and f'{side[0]}_{limb_comp_2}' in device_dict.keys():
                joint_stream = stream_quat2euler_joint(device_dict[f'{side[0]}_{limb_comp_1}'], device_dict[f'{side[0]}_{limb_comp_2}'])
                ...
                for dtype in joint_stream.dtype.fields:
                    if dtype != 'elapsed_s':
                        ...
                        joint_component_list.append(f'{side[0]}_{joint}_joint fquat2euler {dtype} {side[0]}_{joint}')
...
def regs_convert_uV(data, regs, channel, gain=None, position = 'first'):
    ...
    try:
        ...
        gain = { 0:6, 1:1, 2:2, 3:3, 4:4, 5:8, 6:12, 8:0 }[(regval >> 4) & 0b1111] if gain is None else gain # Needed 0b1111 to distinguish between gain of 6 and OFF
...
def get_current(loff_regval):
    ...
    if ((loff_regval>>4) & 0b1) :
        print(f'WARNING: Impedance measurement was made with pullup/pulldown resistor mode. This is less accurate. (regval: {loff_regval})')

    # First/Second Bits: Check that they are valid
    if ((loff_regval & 0b11) == 2):
        print(f'WARNING: Not valid values for the lead-off frequency (first two bits). (regval: {loff_regval})')

    # 6th/7th/8th Bits: Check that max LOFF current accuracy levels are set
    if ((loff_regval>>5) & 0b111) > 0:
        print(f'WARNING: The LOFF Comparator Threshold value (6th-8th bits) is not set to max possible accuracy. (regval: {loff_regval}) ')

    # --- Get Current ---
    # Second bit: DC or AC
    if loff_regval == 0:
        print(f'WARNING: current is off. Cannot calculate impedance. (regval: {loff_regval})')
        raise ValueError
        ...
        print(f'Current from registers is: {current}nA DC, will be using 2x that in imp calculations: {current*2}nA AC. (regval: {loff_regval})')
        current = 2.0 * current # Assume manual FLIP method (eg. 24nA DC --> 48nA AC peak-to-peak)
        return current
...
def load_streams(npz, df=None, convert=True, degrees=False, **kwargs):
    ...
    results = []
    for (stm, pos, lbl, pth, dev, sgm) in zip(df['stream'],
                                              df['position'],
                                              df['label'],
                                              df['path'],
                                              df['device'],
...
def compute_signals(data_dict, regs_dict, streams, times, fil, rms, fft, scales, chunk_length = None):

    signals = { 'sig':[], 'rms':[], 'fft':[], 'rms_chunks':[], 'med_freq_chunks':[], 'time_chunks':[] }
    legends = { 'sig':[], 'rms':[], 'fft':[], 'rms_chunks':[], 'med_freq_chunks':[], 'time_chunks':[] }
    ...
    for stream_name in streams:
        ...
        if stream == "emg":
            ...
            output = output*scale
            sig_arr = np.array(list(tuple(zip(output, elapsed))), dtype={'names':('stream', 'elapsed_s'), 'formats' : ('f8', 'f8')})
            ...
            if fft == 'cdf':
                ...
                fft_arr = np.array(list(tuple(zip(y, f))), dtype={'names':('probability', 'Hz'), 'formats' : ('f8', 'f8')})
            else:
                y = dsp.dB_scale(data_fft)
                fft_arr = np.array(list(tuple(zip(y, f))), dtype={'names':('db', 'Hz'), 'formats' : ('f8', 'f8')})

            # calculate RMS on filtered signal
            rms_out = moving_avg_rms(output, rms)
            rms_arr = np.array(list(tuple(zip(rms_out, elapsed))), dtype={'names':('uv', 'elapsed_s'), 'formats' : ('f8', 'f8')})

            # calculate FFT chunks on filtered signal
            if chunk_length:
                nPts = chunk_length.value *fil['sampling_rate']
                mean_freq_chunks, med_freq_chunks, time_chunks = dsp.calc_freq_chunks(output, elapsed, nPts, fil['sampling_rate'])
                freq_chunks_arr = np.array(list(tuple(zip(mean_freq_chunks, med_freq_chunks, time_chunks))), dtype={'names':('Mean_Hz','Median_Hz', 'elapsed_s'), 'formats' : ('f8', 'f8', 'f8')})
            ...
            legends['fft'].append(f"{ln}: Mean {mean_freq:.1f}Hz, Median {med_freq:.1f}Hz")
            if chunk_length:
                signals['med_freq_chunks'].append(freq_chunks_arr)
                legends['med_freq_chunks'].append(chunks_position_legend or chunks_device_legend)
        else:
            output = output*scale
            sig_arr = np.array(list(tuple(zip(output, elapsed))), dtype={'names':('stream', 'elapsed_s'), 'formats' : ('f8', 'f8')})
            signals['sig'].append(sig_arr)
...
def csv_limb_streams(df, skew):
    ...
    if 'limb' in df:
        for limb in set(df['limb']):
            frame = df[df['limb']==limb].copy() #explictly ask for copy to avoid warning
            if len(frame) == 0:
                continue
            # subset of streams that are not all zeroes
            comp = [ x for x in frame.keys() if x not in excluded and not (frame[x] == 0).all() ]
...
def csv_signals(data_dict, streams, times, scales=None, align=True):
    ...
    for stream_name in streams:
        ...
        if len(parts) == 3:
            ...
            frame = frame[frame['limb']==limb].copy() #explictly ask for copy to avoid warning
        else:
            (stream, comp) = parts
...
def calc_impedance(raw_data: np.array,
                   window_size: int,
                   num_windows: int,
                   regs_dict,
                   ch,
    ...
    data = np.reshape(data[:int(window_size * num_windows)],
...
def stream_splits_to_matrix(stream_data, splits, ch_field, n_interp, paired_splits=False):
    """
    ...
    for idx in range(n_splits):
        ...
        split_data_interp = np.interp(interp_time, split_data["elapsed_s"], split_data[ch_field])
        matrix.append(split_data_interp)

    return np.array(matrix)

def get_sub_stream(stream, start_timestamp, stop_timestamp):
    stream = stream[(stream["elapsed_s"] >= start_timestamp) & (stream["elapsed_s"] <= stop_timestamp)]
    return stream
