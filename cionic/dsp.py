...
def calculate_welch_density(time_data, sampling_freq, nperseg_value = 1024):
    ...
    """
    freq, data_welch_PSD  = signal.welch(time_data, sampling_freq , nperseg = nperseg_value, scaling = "density", nfft = nperseg_value*10)
...
def calculate_welch_spectrum(time_data, sampling_freq, nperseg_value = 1024):
    """
        Calculate Welch Power Spectrum Density (PSD) on linear scale
            ...
            Outputs: numpy arrays
    """
    freq, data_welch_power  = signal.welch(time_data, sampling_freq , nperseg = nperseg_value, scaling = "spectrum", nfft = nperseg_value*10)
...
def find_peaks_algo1(data):
    ...
    """
    peaks, _ = signal.find_peaks(data, height = (None, None), prominence = (None, None), threshold = (None, None))
...
def find_heights_algo1(time, data, peaks, troughs):
    ...
    """
    if peaks[0] > troughs[0]: # if trough comes first: remove 1st index so peak is first (NOTE: not sure this matters so prob delete in future)
        troughs = troughs[1:]
...
def find_peaks_algo2(data):
    ...
    peaks, _ = signal.find_peaks(data_1st_deriv , height = 100, prominence = (None, None), threshold = (None, None))
...
def processRawData(time_raw_us, data_bits, v_ref, channel_gain):
    ...
    time_sec = (time_raw_us -  time_raw_us[0])/ 1000000.

    # Convert data from bits to voltage
    data_uV = 1e6 * data_bits * v_ref / (channel_gain * (2**23 - 1)) # units: microvolts (uV)

    # Return time (sec) and data (uV)
    return time_sec, data_uV  #these are pandas Series (RMS and FFT data are numpy arrays). Should probably pick one format and convert all others to it.
...
def setGain(which_channel, gainlevel, isRLD):
    ...
    if which_channel == 'c2_nomux':
        if isRLD == 1:
            channel_gain = gainlevel #***** NEED TO CONFIRM THIS BASED ON EXPERIMENTS *******
        else:
    ...
    if isRLD == 1:
        print("This in an RLD channel. Ignore its actual name, which is: ", which_channel, ". The gain is now: ", channel_gain)
    else:
...
def check_for_missing(dataframe, sampling_freq, readhex = 1 ):
    ...
    if (not isinstance(dataframe, pd.DataFrame)) and dataframe == []:
        print('Dataframe is empty') #**** NOTE: THIS CHECKING OF EMPTY DATAFRAME SHOULD BE COMBINED WITH PLOTPREP() AND PLOTS SHOULD CLEAR IF EMPTY
    else:
        nRows = len(dataframe)
        # Pull out time
        time = dataframe.time / 1000000. # units: seconds (s)

        # -------- Time Array -----------
        time_intervals = 1/sampling_freq  # units: seconds (s)
        calc_diffs_btw_timestamps = np.array([t - s for s, t in zip(time, time[1:])])
        where_timestamps_notExpected = np.where(((time_intervals-1 > calc_diffs_btw_timestamps) | (calc_diffs_btw_timestamps  > time_intervals+1))) # & (calc_diffs_btw_timestamps != 3))[0]
        ...
        for var in where_timestamps_notExpected:
            number_of_millisec_skipped[str(str(time[var]-time[0]) + 'ms, ' + 'Row #' + str(var))] = str(calc_diffs_btw_timestamps[var]) + 'ms skipped'
        ...
        if readhex == 0:
            ...
            where_ids_not1 = np.where(calc_diffs_btw_ids != 1) #see if any are not == 1
            if np.array(where_ids_not1).size > 0:
                print(" ")
                print("*** IMPORTANT ***")
                print("There are missing ID counts after the following IDs: {}".format(where_ids_not1))
