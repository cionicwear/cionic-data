...
def run_collections(notebook, orgid, study, collections, outdir,
                    prepare_only=False, overwrite=False, limit=None,
                    parameters={}):
    ...
    for collection in collections:
        ...
        if not os.path.exists(nbk_out):
            ...
            parameters['download'] = f"{orgid}/collections/{collection['xid']}/streams/npz"
            ...
            try:
                pm.execute_notebook(notepath, nbk_out, parameters=parameters, prepare_only=prepare_only, cwd=col_dir, store_widget_state=True)
                subprocess.call(['jupyter', 'trust', nbk_out])

            except Exception as e:
                display(Markdown(f"⚠️ **Warning**: Exception in running `{nbk_out}` {e}"))
